...
function BalanceChart({ series, view, gran, range, dailyTarget, c }: {
  series: DayData[], view: string, gran: string, range: number, dailyTarget: number, c: C
}) {
  const W = 352, H = 196, pl = 34, pr = 8, pt = 12, pb = 24
  const iw = W - pl - pr, ih = H - pt - pb
  const effView = range === 0 ? 'cumulative' : view
  const n  = series.length
  const per = dailyTarget * (gran === 'weekly' ? 7 : 1)

  const txtEl = (x: number, y: number, str: string, anchor: string, col?: string, size?: number) => (
    <text key={`t${x}${y}${str}`} x={x} y={y} textAnchor={anchor as 'start' | 'middle' | 'end'} fill={col || c.onSurfVar}
  ...
  if (effView === 'cumulative') {
    ...
    ;[0, 0.5, 1].forEach((t, k) => {
      ...
      els.push(txtEl(pl - 5, y + 3, Math.round(v / 1000) + 'k', 'end'))
    })
    ...
    els.push(txtEl(X(0), H - 7, series[0].md, 'start'))
    els.push(txtEl(X(n - 1), H - 7, series[n - 1].md, 'end'))
    ...
    ;[ymax, (ymax + ymin) / 2, ymin].forEach((v, k) => {
      els.push(txtEl(pl - 5, Y(v) + 3, Math.round(v / 100) / 10 + 'k', 'end', undefined, undefined))
    })
    ...
    els.push(txtEl(W - pr, Y(per) - 4, '目標 +' + Math.round(per), 'end', c.onSurfVar, 8.5))
    els.push(<line key="z" x1={pl} x2={W - pr} y1={y0} y2={y0} stroke={c.onSurf} strokeWidth={1.4} />)
    if (n <= 12) series.forEach((x, i) => els.push(txtEl(X(i), H - 7, x.md.split('/')[1], 'middle', c.onSurfVar, 8)))
    else { els.push(txtEl(X(0), H - 7, series[0].md, 'start')); els.push(txtEl(X(n - 1), H - 7, series[n - 1].md, 'end')) }
  }
...
function GroupedChart({ days, dark, c }: { days: DayData[], dark: boolean, c: C }) {
  const W = 352, H = 140, pl = 6, pr = 6, pt = 8, pb = 20
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} width="100%" style={{ display: 'block', overflow: 'visible' }}>
      ...
      <line x1={pl} x2={W - pr} y1={pt + ih} y2={pt + ih} stroke={c.outlineVar} strokeWidth={1} />
      {days.map((x, i) => {
...
export default function BalanceTab({ s, set, c, data, dailyTarget }: TabProps) {
  ...
  const aggregate = (src: DayData[]) => {
    if (s.gran !== 'weekly') return src
    const w: DayData[] = []
    for (let i = 0; i < src.length; i += 7) {
      const ch = src.slice(i, i + 7)
      w.push({ ...ch[0], d: ch.reduce((a, b) => a + b.d, 0), intake: ch.reduce((a, b) => a + b.intake, 0), burn: ch.reduce((a, b) => a + b.burn, 0) })
    }
    return w
  }
  const series       = aggregate(rawWindow())
  const rw           = rawWindow()
  ...
  const chipSt = (active: boolean) => ({
    border: `1px solid ${active ? 'transparent' : c.outlineVar}`,
    borderRadius: 999,
    background: active ? c.secondaryC : 'transparent',
    color:       active ? c.onSecC : c.onSurfVar,
    fontSize: 12, fontWeight: 500, fontFamily: 'inherit', cursor: 'pointer', padding: '7px 12px',
  })
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14, animation: 'fadeUp .35s ease both' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 12, marginTop: 8 }}>
        <div style={{ display: 'flex', border: `1px solid ${c.outlineVar}`, borderRadius: 999, overflow: 'hidden', height: 40 }}>
          <button type="button" onClick={() => set({ gran: 'daily' })} style={segSt(s.gran === 'daily')}>
            <span className="ms" style={{ fontSize: 18, visibility: s.gran === 'daily' ? 'visible' : 'hidden' }}>check</span>
            日次
          </button>
          <button type="button" onClick={() => set({ gran: 'weekly' })} style={segSt(s.gran === 'weekly')}>
            <span className="ms" style={{ fontSize: 18, visibility: s.gran === 'weekly' ? 'visible' : 'hidden' }}>check</span>
            週次
          </button>
        </div>
      ...
      <div style={{ background: c.surfLow, borderRadius: 24, padding: '18px 16px 14px' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 4, padding: '0 4px' }}>
          <div>
            <div style={{ fontSize: 14, fontWeight: 600 }}>カロリー収支</div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 0 }}>
            ...
            <span style={{ fontSize: 11, color: c.onSurfVar, fontFeatureSettings: '"tnum"', minWidth: 80, textAlign: 'center' }}>{balLabel}</span>
            {arrowBtn(balNextDis, () => set({ balOff: s.balOff - 1 }), 'next', c)}
          </div>
        </div>
        <div style={{ margin: '6px 0 2px' }}>
          <BalanceChart series={series} view={s.view} gran={s.gran} range={s.range} dailyTarget={dailyTarget} c={c} />
        </div>
      ...
      <div style={{ background: c.surfLow, borderRadius: 24, padding: '18px 16px 14px' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8, padding: '0 4px' }}>
          <span style={{ fontSize: 14, fontWeight: 600 }}>摂取 と 消費</span>
          <div style={{ display: 'flex', alignItems: 'center', gap: 0 }}>
            {arrowBtn(grpPrevDis, () => set({ grpOff: s.grpOff + 1 }), 'prev', c)}
            <span style={{ fontSize: 11, color: c.onSurfVar, fontFeatureSettings: '"tnum"', minWidth: 80, textAlign: 'center' }}>{grpLabel}</span>
      ...
      <div style={{ background: c.surfLow, borderRadius: 24, padding: '18px 18px 16px' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
          <span style={{ fontSize: 14, fontWeight: 600 }}>黒字カレンダー</span>
          <div style={{ display: 'flex', alignItems: 'center', gap: 0 }}>
            {arrowBtn(calPrevDis, () => set({ calOff: s.calOff + 1 }), 'prev', c)}
            <span style={{ fontSize: 12, color: c.onSurfVar, fontFeatureSettings: '"tnum"', minWidth: 84, textAlign: 'center' }}>
              {calYr}年{calMon + 1}月
            </span>
      ...
      <div style={{ background: c.surfLow, borderRadius: 24, padding: '18px 18px 16px' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
          <span style={{ fontSize: 14, fontWeight: 600 }}>PFCバランス</span>
          <div style={{ display: 'flex', alignItems: 'center', gap: 0 }}>
            {arrowBtn(pfcPrevDis, () => set({ pfcOff: s.pfcOff + 1 }), 'prev', c)}
            <span style={{ fontSize: 12, color: c.onSurfVar, fontFeatureSettings: '"tnum"', minWidth: 62, textAlign: 'center' }}>{pfcDay?.md}</span>
